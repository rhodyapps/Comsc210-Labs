...
#
#         Description:   QuickSort Algorithm in MIPS Assembly Language
#         
#         Programmer: Tom Newman
#               Date: 2/2022
#
# 	Based on the MIPS Algorithm Code Authored by: Bharath Karumudi     
# 	
#
################################################################################
#
...
#			The sorted array is printed in the console
#
#
####################################  Data Segment #############################
...
DisplayOutput:
		...
		la 	$a2, array	# Load address pointer - this will be used as the base address
		# li	$t0, 0	 	# Clear sum
		li	$t1, 8		# Initialize loop count
	
		li $v0, 4	# message 'Sorted Array'
		la $a0, prompt
		syscall
	...
	arrayLoop:	
		...
		la $a0, ($t2)	# $t2 has the integer of the current array cell
		syscall
		
		li $v0, 4	# print string to have each value print on a new line
		la $a0, crlf
		syscall
		
		addi	$a2, $a2, 4	# Increment address pointer (4 bytes per word)
		addi	$t1, $t1, -1	# Decrement the loop counter
		bgtz	$t1, arrayLoop 	# if ($t1 > 0) branch  (keep looping till counter = 0)
		
	
		addiu, $sp, $sp, -4	# retrun stack space	
		sw $t0, 0($sp)
