...
#####################################################################
#  Register Usage
#	$t1 - the register holding the address of the message
#
#
################ Data Section #######################
...
while:  
	lb      $t0, ($t1)  	# load byte: get the current character into $t0
...
next:  addi $t1, $t1, 1    # increment $t1 to point at the next character in the buffer
   # j while               # /endwhile 
   b while
